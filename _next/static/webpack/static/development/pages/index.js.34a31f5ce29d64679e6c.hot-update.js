  ...
  function processFrame(frame) {
    if (frame < data.length && frame < finish_line - 1) {
      ...
      for (var i = 0; i < panels.length; i++) {
        ...
        var accuracy = Math.round((panel[0] + panel[2]) / (panel[0] + panel[1] + panel[2] + panel[3]) * 1000) / 100; // precision tp / (tp + fp)

        var precision = (panel[0] / (panel[0] + panel[1]) * 1000 / 100).toFixed(1); // recall tp / (tp + fn)

        var recall = Math.round(panel[0] / (panel[0] + panel[3]) * 1000) / 100; // 4 5 6
