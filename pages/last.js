  ...
  useEffect(() => {
    ...
    if (counter % interval === 0 && si.current < 10000 - 1) {
      setFlow(function(prev) {
        ...
        // crx.strokeStyle = scheme.fg
        // crx.strokeRect(0.5, 0.5, cr.width - 1, cr.height - 1)
        for (let a = 0; a < accuracies.length; a++) {
          let p = accuracies[a]
          let y = a * rlh
          // crx.beginPath()
          // crx.moveTo(0.5, y + 0.5)
          // crx.lineTo(cr.width - 1, y + 0.5)
          // crx.stroke()
          ...
          .map(
            ...
            "></div>
              <div style="width: 100%;">${i +
                1} <span style="position: relative; display: inline-block; background: ${
              bgs[p[0]]
            }; padding-right: 0.5ch; padding-left: 0.5ch;">${names[p[0]]}</div> 
    ...
    fetch('sampled.json')
      ...
      .then(data => {
        ...
        for (let i = 0; i < readouts.length; i++) {
          ...
          $border.style.height = panel[3] + rlh * 3 + 1 + 'px'
